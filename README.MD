...
<p><code>BOT_URL=</code></p>
   
   - TOKEN - токен вашего бота, ADMIN_GROUP_ID - ID чата (группы) в Telegram, в который он будет пересылать сообщения администрации (Как узнать ID чата telegram: https://dzen.ru/a/Y8mvM0IEN0exrcSu), FLASK_SITE_ADDRESS - адрес сайта - админ панели (http/https), BOT_URL - ссылка на бота.
   - Необходимо после = ввести соотвутствующие данные.

5. Поменяйте администратора во вкладке "admin_management" (стандартный логин: admin, пароль: admin)
...
<p><code>docker build -t stud-bot .</code></p>

<p><code>docker run stud-bot</code></p>
